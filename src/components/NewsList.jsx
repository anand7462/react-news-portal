...
const NewsList = ({ posts, totalPosts, postsPerPage, paginate, addToFavorites }) => {
  const pageNumbers = [];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {posts.map((post, index) => (
          <div key={index} className="p-4 border rounded shadow-sm bg-white relative flex flex-col justify-between">
            ...
            <button onClick={() => addToFavorites(post)} className="mt-4 bg-red-600 text-white px-2 py-1 rounded">
              Add to Favorites
            </button>
          </div>
        ))}
